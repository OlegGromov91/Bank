...
export class TransferWithinSystemByCardDialogComponent implements OnInit {
  ...
  public transferMoneyToUserSelf(): void {
    ...
    this.transferService.transferMoneyToUserSelf(this.data.get('userId')!, this.data.get('cardId')!, this.transferToUserSelfForm.value).toPromise().then(() => {
      this.close();
    }).catch((err: HttpErrorResponse) => {
      this.snackBar.open(JSON.stringify(err.error), "", {
        horizontalPosition: "end",
  ...
  private showUserCards(): void {
    this.cardService.showCards(this.data.get('userId')!).subscribe(data => {
      this.cardsEntry = data;
    });
  }

  private showUserBankAccounts(): void {
    this.bankAccountService.findAllBankAccountsByUserId(this.data.get('userId')!).subscribe(data => {
